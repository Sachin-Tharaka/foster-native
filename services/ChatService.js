class ChatService {
  ...
  async fetchMessagePreviewsUser(token, userId) {
    try {
  ...
  async fetchMessagePreviewsAgent(token, kennelId) {
    try {
      const response = await fetch(
        `${this.baseUrl}/api/chat/get-chat-previews-by-kennel?kennelId=${kennelId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
